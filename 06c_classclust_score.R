...
clust_source_dir = paste(clust_dir,  "/source", sep=""); dir.create (clust_source_dir,showWarnings=F)
  ...
  tryCatch ({
    ...
    if (dist_type!="NA") {
      ...
      if (dist_type!="NA") {
        ...
        # } else { 
        nca = NCA_score(as.matrix(d), rowlabel_df)$p
        names(nca) = "nca"
    ...
    for (clust_path in dist_paths[[dist_type]]) {
      ...
      rowlabel = meta_file[match(names(rowclust),meta_file[,target_col]),target_col]
